"""Differentiable future-bond supervision for the sequential R4 DiT study.

The frozen codec is intentionally used with autograd enabled: its parameters
remain frozen, while the endpoint prediction retains its gradient path back to
the DiT and its adapter.  This module owns only the round-2 auxiliary loss; it
does not alter codec topology, latent statistics, or the RF objective.
"""
...
def observed_frame_mask(batch: DiTLatentBatch) -> Tensor:
