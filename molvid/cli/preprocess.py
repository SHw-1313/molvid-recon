#!/usr/bin/env python
"""Build ordered ATLAS/MISATO clip stores for the current codec.

Example (the paths are intentionally supplied by the operator)::

    python -m molvid.cli.preprocess atlas \
      --root /data1/repo/BioKinema/data_atlas \
      --output-root /path/to/clip_store
...
from ..data.store import ClipMMapWriter
...
from ..data.manifest import source_inventory as _inventory, write_split_manifest
...
def main(argv: list[str] | None = None) -> None:
    ...
    manifest = write_split_manifest(
        ...
        source_version=(
            ...
            else "MISATO Zenodo record 7711953; native dt recorded in migration evidence"
        ),
