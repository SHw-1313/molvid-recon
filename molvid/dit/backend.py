"""Reference and vectorized factorized DiT execution over static block layouts."""
...
def _atom_temporal(
    h: Tensor, v: Tensor, batch: LatentBatch, attention: Any
) -> tuple[Tensor, Tensor]:
    h_atoms = h.permute(1, 0, 2)
    v_atoms = v.permute(1, 0, 2, 3)
    valid = batch.token_mask.index_select(0, batch.abid)
...
def _build_block_groups(batch: LatentBatch) -> tuple[list[Tensor], list[int]]:
...
def reference_block_forward(
    ...
    spatial_h, spatial_v = _block_spatial(
        h_norm, v_norm, batch, block.spatial, block_groups, block_samples
    )
    h = h + torch.tanh(h_gate) * spatial_h
    v = v + torch.tanh(v_gate).unsqueeze(-2) * spatial_v

    h_norm, v_norm, h_gate, v_gate = block.temporal_adaln(h, v, condition)
    temporal_h, temporal_v = _atom_temporal(h_norm, v_norm, batch, block.temporal)
    h = h + torch.tanh(h_gate) * temporal_h
    v = v + torch.tanh(v_gate).unsqueeze(-2) * temporal_v
