...
from .geometry import _temporal_metrics, aligned_rmsf_metrics
...
def rmsf(
    prediction: Tensor, target: Tensor, batch: Any, *, frames: Sequence[int] | None = None
) -> dict[str, Any]:
    """Aligned RMSF with the original sample-equal aggregation."""

    return aligned_rmsf_metrics(prediction, target, batch, frames=frames)


def velocity_metrics(
    prediction: Tensor, target: Tensor, batch: Any, frames: Sequence[int]
) -> dict[str, Any]:
    metrics = _temporal_metrics(prediction, target, batch, frames)
    return {"velocity_rmse": metrics["velocity_rmse"], "dynamic": metrics["dynamic"]}


def acceleration_metrics(
    prediction: Tensor, target: Tensor, batch: Any, frames: Sequence[int]
) -> dict[str, float]:
    return {"acceleration_rmse": _temporal_metrics(prediction, target, batch, frames)["acceleration_rmse"]}


def fft_retention(
    prediction: Tensor, target: Tensor, batch: Any, frames: Sequence[int]
) -> float:
    return float(_temporal_metrics(prediction, target, batch, frames)["frequency_retention"])
