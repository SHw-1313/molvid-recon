...
from molvid.data.store import ClipMMapDataset, ClipMMapWriter, StaticClipDataset, static_record_to_clip
...
def test_static_dataset_wraps_existing_mmap_store(tmp_path):
    ...
    (root / "index.txt").write_text(f"mol_0\t0\t{len(payload)}\t3\n")
