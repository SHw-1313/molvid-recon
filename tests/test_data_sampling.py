...
from molvid.data.sampling import (
    ClipItemSpec,
    ClipSpecTable,
...
def test_trajectory_cap_resume_preserves_window_selection():
    specs = ClipSpecTable.from_specs(
        [
            ClipItemSpec(
                index=trajectory * 62 + window,
                atoms=3,
                frames=16,
                task="trajectory",
                time_bucket_id="dt_100ps",
                sample_id=f"traj{trajectory}_w{window:02d}",
            )
            for trajectory in range(2)
            for window in range(62)
        ]
    )
    original = TrajectoryCappedBatchSampler(
        specs, max_tokens=96, clips_per_trajectory=4, seed=79
    )
    original.set_epoch(3)
    selected = original.selected_sample_ids
    batches = list(original)
    assert len(selected) == 8
    assert len(set(selected)) == 8
    assert sum(sample.startswith("traj0_") for sample in selected) == 4
    assert sum(sample.startswith("traj1_") for sample in selected) == 4
    assert sorted(index for batch in batches for index in batch) == sorted(
        item.index for item in specs if item.sample_id in selected
    )
    resumed = TrajectoryCappedBatchSampler(
        specs, max_tokens=96, clips_per_trajectory=4, seed=79
    )
